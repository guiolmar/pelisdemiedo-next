...
export default function Home() {
    ...
    const fetchTrendingMovies = async () => {
      try {
        const response = await axios.get('/api/tmdb', {
          params: { type: 'trending/movie/week', "query[language]": 'es-ES' },
        });
  ...
  useEffect(() => {
    ...
    const fetchPopularMovies = async () => {
      try {
        const response = await axios.get('/api/tmdb', {
          params: { type: 'discover/movie', "query[language]": 'es-ES', "query[with_genres]": 27 },
        });
        ...
        <div className="absolute inset-0 bg-black bg-opacity-65 flex flex-col items-center justify-center text-white p-6">
          ...
          <div className="w-full max-w-md relative">
            ...
            <Input
              ...
              className="text-base w-full pl-10 pr-4 py-2 rounded-full bg-white text-black placeholder-gray-300 focus:outline-none focus:ring-2 focus:ring-white"
            />
